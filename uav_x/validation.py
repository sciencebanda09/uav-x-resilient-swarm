...
def monte_carlo(seeds: list[int], duration: float = 120.0) -> dict:
...
def main() -> None:
    p = argparse.ArgumentParser(); p.add_argument("--out", default="reports/validation.json"); p.add_argument("--duration", type=float, default=30.0); a = p.parse_args()
    result = {"monte_carlo": monte_carlo([7, 17, 27], a.duration), "packet_loss_sensitivity": sensitivity("packet_loss", [0.0, .05, .1, .2], duration=a.duration)}
